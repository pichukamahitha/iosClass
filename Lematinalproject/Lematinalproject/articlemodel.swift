...
struct articelcatagoriessmodel: Codable {
    let status : String?
    let status_code : Int?
    let message : String?
    let api : String?
    let results : [articlecategoriesResults]?
    let current_page : Int?
    let total_pages : Int?
    ...
    init(from decoder: Decoder) throws {
        ...
        message = try values.decodeIfPresent(String.self, forKey: .message)
        api = try values.decodeIfPresent(String.self, forKey: .api)
        results = try values.decodeIfPresent([articlecategoriesResults].self, forKey: .results)
        current_page = try values.decodeIfPresent(Int.self, forKey: .current_page)
        total_pages = try values.decodeIfPresent(Int.self, forKey: .total_pages)
    }

}
struct articlecategoriesResults : Codable {
    let title : String?
    let iD : Int?
    let author_name : String?
    let image_url : String?
